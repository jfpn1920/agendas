...
Module actualizar_contacto
    ...
    Sub ActualizarContacto()
        ...
        MostrarContactos()
        Console.Write(vbCrLf & "Ingresa el ID del contacto a actualizar: ")
        ...
        Try
            Using conexion As MySqlConnection = ObtenerConexion()
                conexion.Open()
                Dim query As String = "UPDATE contactos SET nombre=@nombre, telefono=@telefono, correo=@correo WHERE id=@id"
                Using comando As New MySqlCommand(query, conexion)
                    comando.Parameters.AddWithValue("@nombre", nombre)
                    comando.Parameters.AddWithValue("@telefono", telefono)
                    comando.Parameters.AddWithValue("@correo", correo)
